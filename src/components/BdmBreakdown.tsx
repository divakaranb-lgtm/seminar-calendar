...
export default function BdmBreakdown({ seminars }: { seminars: Seminar[] }) {
  ...
  return (
    <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      <h3 className="mb-1 text-base font-semibold text-slate-900">BDM Breakdown</h3>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[420px] text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-50">
            {rows.map((row) => (
              <tr key={row.bdm}>
                <td className="py-2 font-medium text-slate-800">{row.bdm}</td>
                <td className="py-2 text-right text-slate-700">{row.sessions}</td>
                <td className="py-2 text-right text-slate-700">{formatStat(row.students)}</td>
                <td className="py-2 text-right text-slate-700">{formatStat(row.prospects)}</td>
                <td className="py-2 text-right text-slate-700">{formatStat(row.futureIntake)}</td>
              </tr>
            ))}
          </tbody>
        </table>
